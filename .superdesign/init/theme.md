...
/* =============================================================================
   ...
   primary + teal accent, first-class light/dark.
   Theming contract:
   - :root holds light tokens (default).
   - OS preference applies dark automatically unless the user forced light.
   - [data-theme] on <html> always wins over the OS.
============================================================================= */
...
h1 { font-size: 1.6rem; font-weight: 750; letter-spacing: -0.015em; }
...
/* Below ~480px there isn't room for brand + profile name + both toggles +
   sign out on one line, and wrapping mid-row looks broken rather than
   responsive — drop the profile name first since it's the least essential. */
@media (max-width: 480px) {
  .topbar { flex-wrap: nowrap; gap: 0.4rem; padding: 0.6rem 0.75rem; }
...
.student-preview-banner strong { color: var(--text); }
...
.run-deck-host .run-deck-frame { width: 100%; }
.run-deck-warning {
  position: absolute;
  z-index: 4;
  top: 0.75rem;
...
.run-join-card.compact {
  padding: 0.8rem;
}
.run-join-card.compact h2 {
  font-size: 1rem;
}
.run-join-card a {
...
.checkpoint-options .pulse-choice {
  min-height: 44px;
...
.lang-toggle:hover { color: var(--text); background: var(--surface-2); }

/* ---------------------------------------------------------------- live pulses */
/* Student choices: big tap targets, one decision per screen. */
.pulse-choice {
  display: block;
  width: 100%;
  text-align: left;
...
.pulse-verdict.good { color: var(--good); }
.pulse-verdict.bad { color: var(--danger); }

/* Instructor / projector distribution bars. */
.pulse-bar {
  position: relative;
  display: flex;
  align-items: center;
...
.pulse-bar.correct { border-color: var(--good); }
.pulse-bar-fill {
  position: absolute;
  inset: 0 auto 0 0;
...
.pulse-bar.correct .pulse-bar-fill { background: var(--good-soft); }
.pulse-bar-label { position: relative; z-index: 1; flex: 1; font-weight: 600; }
.pulse-bar-count {
